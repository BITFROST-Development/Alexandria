...
class PaperStylePickerViewController: UIViewController, NewNotebookPickerDelegate, NotebookImageStyle {
    ...
    @IBAction func donePicking(_ sender: Any){
        ...
        controller.selectedPaperName = newSelection
        controller.tableView.beginUpdates()
        (controller.tableView.cellForRow(at: IndexPath(row: 0, section: 0)) as! NewNotebookThumbnail).filePaperStyle.image = newImage
        controller.tableView.reloadRows(at: [IndexPath(row: 3, section: 0)], with: .automatic)
