...
extension BookPreferences{
                            ...
                            if allSet {
                                ...
                                if self.currentBook.title != self.finalFileName{
                                    if self.currentBook.cloudVar.value ?? false {
                                                ...
                                                if successeses {
                                                    do {
                                                        try FileManager.default.moveItem(atPath: self.currentBook.localAddress!, toPath: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].path + "/Books/\(self.finalFileName!).pdf")
                                                            do {
                                                                try realm.write({
                                                                    self.currentBook.title = self.finalFileName!
                                                                    if self.loggedIn {
                                        ...
                                        do {
                                            try FileManager.default.moveItem(atPath: self.currentBook.localAddress!, toPath: FileManager.default.urls(for: .applicationDirectory, in: .userDomainMask).first!.path + "/Books/\(self.finalFileName!).pdf")
                                                do {
                                                    try realm.write({
                                                        self.currentBook.title = self.finalFileName!
                                                        if self.loggedIn {
                                ...
                                if (success){
                                    ...
                                    do {
                                        try realm.write({
                                            ...
                                            user.alexandriaData?.cloudBooks.append(self.currentBook)
                                            user.alexandriaData?.localBooks.remove(at: self.currentBookIndex)
                                            completion(true)
                                        })
                                    } catch let error {
                                        print(error.localizedDescription)
                ...
                if currentBook.cloudVar.value ?? false {
                    ...
                    if currentBook.localAddress == nil {
                        ...
                        alert.addAction(UIAlertAction(title: "Local Copy", style: .cancel, handler: { _ in
                            GoogleDriveTools.retrieveBookFromDrive(id: self.currentBook.id!, name: self.currentBook.title! + ".pdf", service: GoogleDriveTools.service){ success in
                                if success{
                                    GoogleDriveTools.deleteFile(service: GoogleDriveTools.service, id: self.currentBook.id!, local: false)
                                    let hashTables = realm.objects(BookToListMap.self)
                                    ...
                                    do {
                                        try realm.write({
                                            ...
                                            let user = realm.objects(CloudUser.self)[0]
                                            user.alexandriaData?.localBooks.append(self.currentBook)
                                            user.alexandriaData?.cloudBooks.remove(at: self.currentBookIndex)
                                            completion(true)
                                        })
                                    } catch let error {
                                        print(error.localizedDescription)
    ...
    func deleteFile(){
        ...
        if currentBook.localAddress != nil {
            do{
                try FileManager.default.removeItem(atPath: currentBook.localAddress!)
                if currentBook.cloudVar.value ?? false {
                    GoogleDriveTools.deleteFile(service: GoogleDriveTools.service, id: currentBook.id!, local: false){ success in
                        let hashTables = realm.objects(BookToListMap.self)
            ...
            GoogleDriveTools.deleteFile(service: GoogleDriveTools.service, id: currentBook.id!, local: false)
            let hashTables = realm.objects(BookToListMap.self)
